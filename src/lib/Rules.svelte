<script>
    import { getGame } from "./game.svelte";
    const game = getGame();
</script>

<main>
    <div class="hero">
        <div class="cta">
            <div>
                <svg
                    stroke-width="0"
                    viewBox="0 0 256 256"
                    height="4rem"
                    width="4rem"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <path
                        d="M92,24V16a4,4,0,0,1,8,0v8a4,4,0,0,1-8,0ZM16,100h8a4,4,0,0,0,0-8H16a4,4,0,0,0,0,8ZM126.21,35.58a4,4,0,0,0,5.37-1.79l8-16a4,4,0,0,0-7.16-3.58l-8,16A4,4,0,0,0,126.21,35.58Zm-96,88.84-16,8a4,4,0,0,0,3.58,7.16l16-8a4,4,0,1,0-3.58-7.16Zm186.28,62.41a12,12,0,0,1,0,17L203.8,216.49a12,12,0,0,1-17,0l-51.31-51.31a3.93,3.93,0,0,0-3.58-1.11,4,4,0,0,0-2.89,2.27l-17.78,46.31a.77.77,0,0,1-.07.16A11.85,11.85,0,0,1,100.26,220h-.59a11.88,11.88,0,0,1-10.8-8.23L36.6,51.68A12,12,0,0,1,51.68,36.6L211.76,88.87a12,12,0,0,1,1.05,22.33l-.16.07-46.31,17.78a4,4,0,0,0-1.17,6.47Zm-5.66,5.66-51.31-51.32a12,12,0,0,1,3.7-19.49l.16-.06,46.31-17.79a3.95,3.95,0,0,0-.42-7.35L49.2,44.21a4,4,0,0,0-5,5L96.48,209.27a4,4,0,0,0,7.36.42l17.78-46.31a1.11,1.11,0,0,1,.07-.16,12,12,0,0,1,8.76-7,12.21,12.21,0,0,1,2.24-.21,12,12,0,0,1,8.49,3.52l51.31,51.31a4,4,0,0,0,5.65,0l12.69-12.69A4,4,0,0,0,210.83,192.49Z"
                    >
                    </path>
                </svg>
            </div>
            <div class="game-name">Click Me If You Can</div>
            <div class="reflex">A simple game to test your reflexes</div>
            <button onclick={game.start}
                ><svg
                    stroke="currentColor"
                    fill="none"
                    stroke-width="2"
                    viewBox="0 0 24 24"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    height="1rem"
                    width="1rem"
                    class="icon"
                    xmlns="http://www.w3.org/2000/svg"
                    ><path d="M7 4v16l13 -8z"></path></svg
                >Start Game</button
            >
        </div>
        <div class="rules">
            <h4>Game Information</h4>
            <ul>
                <li>Your task is just to click the button.</li>
                <li>
                    Game starts at Level 1, and every 10 seconds, the level goes
                    up.
                </li>
                <li>
                    The button will change positions faster as you level up.
                </li>
                <li>
                    Your score increases by the current level with each click.
                    For example, at level 1, a click adds 1 point, while at
                    level 5, it adds 5 points, reflecting the higher difficulty
                    at higher levels.
                </li>
                <li>
                    The <b class="game-end">Game Ends</b> when you miss clicking
                    the button during a level.
                </li>
            </ul>
        </div>
    </div>
</main>

<style>
    main {
        display: flex;
        justify-content: center;
    }
    .hero {
        padding: 1rem;
        max-width: 600px;
        width: 100%;
        margin-top: 7rem;
    }
    .cta {
        display: flex;
        flex-direction: column;
        justify-content: center;
        align-items: center;
    }
    .game-name {
        margin-top: 1rem;
        font-size: 2rem;
    }
    .reflex {
        color: #555;
    }
    button {
        margin-top: 2rem;
        width: max-content;
    }

    .rules {
        background-color: #edeff2;
        padding: 1rem 2rem;
        margin-top: 5rem;
        border: 1px solid #d3d8de;
    }

    ul {
        margin-top: 0.5rem;
        margin-left: 1rem;
        color: #444;
    }
    li {
        margin-bottom: 4px;
    }
    .icon {
        margin-right: 0.5rem;
    }
</style>
